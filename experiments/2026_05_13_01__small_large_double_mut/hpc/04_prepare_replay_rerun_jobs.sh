...
for rep_id in ${!reps_to_gens[@]}; do
  solution_gen=${reps[$rep_id]}
  final_gen=$((solution_gen - 1))
  gens=${reps_to_gens[$rep_id]}
  for replay_gen in ${gens[@]}; do
    key="${rep_id}x${replay_gen}"
    replay_reps=${rep_gens_to_reps[$key]}
    for replay_rep in ${replay_reps[@]}; do
      echo "$rep_id $replay_gen $replay_rep"
      ...
      sed -i "s/(<REPLAY_GEN>)/${replay_gen}/g" out.sb
      sed -i "s/(<GENS_LEFT>)/$((TOTAL_GENS - replay_gen))/g" out.sb
      sed -i "s/(<REPLAY_REP>)/${replay_rep}/g" out.sb
  
  
      # Move output sbatch file to final destination, and add to roll_q queue if needed
      TIMESTAMP=$( get_timestamp )
      SLURM_FILENAME=${SCRATCH_SLURM_JOB_DIR}/${EXP_SLUG}__rereplay_${rep_id}_${replay_gen}_${replay_rep}__${TIMESTAMP}.sb
