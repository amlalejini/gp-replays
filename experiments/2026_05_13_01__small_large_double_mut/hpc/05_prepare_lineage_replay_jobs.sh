...
TEMPLATE_FILE=05_job_template_lineage_replays.sb
TOTAL_GENS=200
...
declare -A rep_gens
rep_gens[296]="36 37 38 40 41 42 43 44"

for rep_id in ${!reps[@]}; do
  for lineage_gen in ${rep_gens[$rep_id]}; do
    echo "$rep_id $lineage_gen"
    ...
    sed -i "s/(<LINEAGE_GEN>)/${lineage_gen}/g" out.sb
    sed -i "s/(<GENS_LEFT>)/$((TOTAL_GENS - lineage_gen))/g" out.sb
  
  
    # Move output sbatch file to final destination, and add to roll_q queue if needed
    TIMESTAMP=$( get_timestamp )
    SLURM_FILENAME=${SCRATCH_SLURM_JOB_DIR}/${EXP_SLUG}__replay_lineage_${rep_id}_${lineage_gen}__${TIMESTAMP}.sb
